/**
 * LUXE SHOP — Main JavaScript
 * Handles UI interactions, cart, wishlist, search, etc.
 */
...
const Cart = {
  ...
  updateQty(key, qty) {
    const items = this.getItems().map(i => i.key === key ? { ...i, qty: Math.max(1, qty) } : i);
    this.save(items);
  },
...
function initCountdown() {
  ...
  function update() {
    ...
    if (diff <= 0) { el.innerHTML = '<p>Offer ended</p>'; return; }
    ...
    el.innerHTML = `
      <div class="timer-unit"><div class="timer-num">${pad(d)}</div><div class="timer-label">Days</div></div>
      <div class="timer-unit"><div class="timer-num">${pad(h)}</div><div class="timer-label">Hours</div></div>
      <div class="timer-unit"><div class="timer-num">${pad(m)}</div><div class="timer-label">Mins</div></div>
      <div class="timer-unit"><div class="timer-num">${pad(s)}</div><div class="timer-label">Secs</div></div>
    `;
  }
  update();
...
/* ============================================================
   FILTER CHECKBOXES (shop)
   ============================================================ */
function initFilters() {
  $$('.filter-checkbox').forEach(box => {
    box.addEventListener('click', () => {
      box.classList.toggle('checked');
      const label = box.closest('.filter-option')?.querySelector('.filter-label')?.textContent;
      updateActiveFilters();
    });
  });
  ...
  function updateActiveFilters() {
    ...
    $$('.filter-checkbox.checked').forEach(box => {
      ...
      tag.addEventListener('click', () => { box.classList.remove('checked'); updateActiveFilters(); });
...
function renderCartPage() {
  ...
  if (!items.length) {
    container.innerHTML = `
      <div style="text-align:center;padding:4rem 0">
        ...
        <a href="/pages/shop.php" class="btn btn-primary">Explore Products</a>
      </div>`;
    return;
  }
  ...
  container.innerHTML = items.map(item => `
    <div class="cart-item" data-key="${item.key}">
      <img class="cart-item-img" src="${item.image || '/assets/images/placeholder.jpg'}" alt="${item.name}">
      <div>
        ...
        <div class="cart-item-controls">
          <div class="qty-control">
            ...
            <input class="qty-input" type="number" value="${item.qty}" min="1" data-key="${item.key}">
...
function initCartPage() {
  ...
  document.addEventListener('click', e => {
    ...
    if (qtyBtn) {
      const input = document.querySelector(`.qty-input[data-key="${qtyBtn.dataset.key}"]`);
      if (!input) return;
      let val = parseInt(input.value) || 1;
...
function initWishlistPage() {
  ...
  if (!items.length) {
    container.innerHTML = `<div style="text-align:center;padding:4rem 0"><p style="font-size:3rem;margin-bottom:1rem">♡</p><h3>Your wishlist is empty</h3><p style="margin:1rem 0 2rem">Save items you love for later.</p><a href="/pages/shop.php" class="btn btn-primary">Shop Now</a></div>`;
    return;
  }
    ...
    <div class="product-card">
      <div class="product-image-wrap">
        ...
        <div class="product-actions-hover">
          <button class="btn-add-cart" data-action="add-to-cart" data-id="${item.id}" data-name="${item.name}" data-price="${item.price}" data-image="${item.image}">Add to Cart</button>
          <button class="btn-wishlist" data-action="toggle-wishlist" data-id="${item.id}" data-name="${item.name}" data-price="${item.price}">♥</button>
        </div>
      </div>
...
function initScrollReveal() {
  ...
  $$('.product-card, .category-card, .feature-item, .testimonial-card').forEach(el => {
    el.style.opacity   = '0';
